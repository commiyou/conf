    ...
    for line in lines:
        if line.startswith("bag") or "bag:" in line or "bag：" in line:
            ...
            for line_part in line.replace("：", " ").split():
                ...
                elif "/log/" in line_part or "/logs/" in line_part or line_part.endswith("log.gz"):
                    res["log"] = line_part
...
class AllrideJira:
    ...
    def get_todo_issues(self):
        ...
        if res.status_code == 200:
            ...
            issues = [(x.attrs["data-key"], x.attrs["title"]) for x in res.html.find("ol.issue-list li")]
            return issues
        else:
            return []
    ...
    def get_issue(self, issue_key):
        ...
        if res.status_code == 200:
            ...
            issue = dict(key=issue_key, 
                    url=url, 
                    status = status,
                    desc=desc, **extract_issue_desc(desc))
            print(issue)
...
def download_todo_issues(jira):
    ...
    for issue in issues:
        ...
        #if (p/name).exists():
        #    continue
        cmd = f"mkdir -p {p / name} && cd { p / name } && {OSSUTIL64} cp --recursive -u "
        if issue_res["bags"]:
