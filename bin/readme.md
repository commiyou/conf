...
根据指定的键去除或筛选文件中的重复行。功能类似于 `uniq` 但更强大。

**重要提示:**
- 默认模式下，它打印出现次数 **小于** `-n` 指定次数的行（默认 `n=1`，即打印第一次出现的行）。
- 使用 `-r` 或 `--reverse` 参数后，逻辑反转，它会打印出现次数 **大于或等于** `-n` 指定次数的行。
...
- **根据第一列去重，并忽略标题行:**
  ```shell
  cat file.txt | dedup -k 0 --ignore-header
  ```

- **找出重复2次及以上的行 (基于第2和第3列):**
  ```shell
  cat file.csv | dedup -k 1,2 -d ',' -n 2 -r
  ```

- **保留每种键的前3次出现:**
  ```shell
  cat logs.txt | dedup -k 0 -n 3
  ```

- **忽略大小写进行去重:**
  ```shell
  cat names.txt | dedup -i
  ```
